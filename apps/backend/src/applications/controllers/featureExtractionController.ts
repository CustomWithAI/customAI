...
export const featureExtraction = new Elysia({
  ...
  .guard({ response: featureExtractionResponseDto }, (app) =>
    app
      .post(
        "/",
      ...
      .get(
        ...
        { query: paginationDto, response: featureExtractionsResponseDto }
      )
      ...
      .delete("/:id", async ({ user, params, featureExtractionService }) => {
        return featureExtractionService.deleteFeatureExtraction(
          user.id,
          params.id
        );
      })
