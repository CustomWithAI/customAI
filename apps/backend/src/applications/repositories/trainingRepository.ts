...
import { roundUpVersion } from "@/utils/version";
...
export class TrainingRepository {
	...
	public async findLatestByVersion(
		workflowId: string,
		version: string,
		position: "major" | "minor" | "patch",
	) {
		return db
			.select({
				version: trainings.version,
			})
			.from(trainings)
			.where(
				and(
					eq(trainings.workflowId, workflowId),
					lt(
						sql`string_to_array(${trainings.version}, '.')::int[]`,
						sql`string_to_array(${roundUpVersion(version, position)}, '.')::int[]`,
					),
				),
			)
			.orderBy(desc(sql`string_to_array(${trainings.version}, '.')::int[]`))
			.limit(1);
	}
