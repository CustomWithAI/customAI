...
import { useCreateAugmentation } from "@/hooks/mutations/augmentation-api";
import { useCreateDataset } from "@/hooks/mutations/dataset-api";
import { useToast } from "@/hooks/use-toast";
import { useRouter } from "@/libs/i18nNavigation";
import type { AugmentationDetailsSchema } from "@/models/augmentation";
import {
...
const CreateAugmentationPage = () => {
	const { mutateAsync: createAugmentation } = useCreateAugmentation();
	const router = useRouter();
	const { toast } = useToast();

	const onSubmitData = async (data: AugmentationDetailsSchema) => {
		await createAugmentation(
			...
			},
			{
				onSuccess: (ctx) => {
					toast({
						title: `create data augmentation ${ctx?.data.name} successfully`,
					});
	...
	const { Provider, Build } = useFormBuilder({
		...
		formName: `create-augmentation-${new Date().valueOf()}`,
	});
...
export default CreateAugmentationPage;
